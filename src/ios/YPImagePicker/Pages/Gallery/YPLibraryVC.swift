...
public class YPLibraryVC: UIViewController, YPPermissionCheckable {
    ...
    func refreshMediaRequest() {
        ...
        if mediaManager.fetchResult.count > 0 {
            if YPConfig.library.selectedImages != nil && YPConfig.library.selectedImages?.count as! Int > 0 {
                var first = true
                var selectedImagesIndex = 0

                if YPConfig.library.selectedImages?.count as! Int > 1 {
                   multipleSelectionEnabled = true
                }
                v.collectionView.reloadData()

                for image: NSDictionary in YPConfig.library.selectedImages! {
                    if let id = image["id"] as? String {
                        mediaManager.fetchResult.enumerateObjects({ (asset: PHAsset!,
                                                                     count: Int,
                                                                     stop: UnsafeMutablePointer<ObjCBool>) in
                            let selectedImage = image
                            if id == asset.localIdentifier {
                                // print("asset exists: ", asset.localIdentifier)

                                let cropX = selectedImage["cropX"] as? Double ?? 0.0
                                let cropY = selectedImage["cropY"] as? Double ?? 0.0
                                let cropW = selectedImage["cropW"] as? Double ?? 1.0
                                let cropH = selectedImage["cropH"] as? Double ?? 1.0

                                let zoomScale = selectedImage["restoreZoom"] as? Double ?? 1.0
                                let offsetX = selectedImage["restoreOffsetX"] as? Double ?? 0.0
                                let offsetY = selectedImage["restoreOffsetY"] as? Double ?? 0.0

                                let cropRect = CGRect(x: CGFloat(cropX), y: CGFloat(cropY), width: CGFloat(cropW), height: CGFloat(cropH))

                                let contentOffset: CGPoint = CGPoint(x: offsetX, y: offsetY)

                                if YPConfig.library.selectedImages?.count as! Int > 1 {
                                    self.selection.append(YPLibrarySelection(index: count, cropRect: cropRect, scrollViewContentOffset: contentOffset, scrollViewZoomScale: CGFloat(zoomScale)))
                                }
                                //self.currentlySelectedIndex = count
         ...
         //   delegate?.noPhotosForOptions() // close if no videos in gallery
        }
        scrollToTop()
    }
    ...
    public func selectedMedia(photoCallback: @escaping (_ photo: YPMediaPhoto) -> Void,
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if self.multipleSelectionEnabled && self.selection.count > 1 {
                let selectedAssets: [(asset: PHAsset, cropRect: CGRect?, zoomScale: CGFloat?, contentOffset: CGPoint?)] = self.selection.map {
                    return (self.mediaManager.fetchResult[$0.index], $0.cropRect, $0.scrollViewZoomScale, $0.scrollViewContentOffset)
                }
                ...
                for asset in selectedAssets {
                    ...
                    case .image:
                        self.fetchImageAndCrop(for: asset.asset, withCropRect: asset.cropRect) { image, exifMeta in
                            let photo = YPMediaPhoto(image: image.resizedImageIfNeeded(), exifMeta: exifMeta, asset: asset.asset, cropRect: asset.cropRect, zoomScale: asset.zoomScale, contentOffset: asset.contentOffset)
                    ...
                    case .video:
                        self.checkVideoLengthAndCrop(for: asset.asset, withCropRect: asset.cropRect) { videoURL in
                            let videoItem = YPMediaVideo(thumbnail: thumbnailFromVideoPath(videoURL),
                                                         videoURL: videoURL, asset: asset.asset, cropRect: asset.cropRect, zoomScale: asset.zoomScale, contentOffset: asset.contentOffset)
                    ...
                    self.checkVideoLengthAndCrop(for: asset, callback: { videoURL in
                        DispatchQueue.main.async {
                            ...
                            let video = YPMediaVideo(thumbnail: thumbnailFromVideoPath(videoURL),
                                                     videoURL: videoURL, asset: asset, cropRect: self.v.currentCropRect(), zoomScale: self.v.assetZoomableView.zoomScale, contentOffset: self.v.assetZoomableView.contentOffset)
                            videoCallback(video)
                        }
                    })
                case .image:
                    self.fetchImageAndCrop(for: asset) { image, exifMeta in
                        DispatchQueue.main.async {
                            ...
                            let photo = YPMediaPhoto(image: image.resizedImageIfNeeded(),
                                                     exifMeta: exifMeta,
                                                     asset: asset,
                                                     cropRect: self.v.currentCropRect(), zoomScale: self.v.assetZoomableView.zoomScale, contentOffset: self.v.assetZoomableView.contentOffset)
                            photoCallback(photo)
                        }
                    }
                case .audio, .unknown:
