...
//                        let mime_type = self.mimeTypes[file_url.pathExtension.lowercased()]!
    ...
    private func parseId(id: String) -> [String] {
        return id.components(separatedBy: ";")
    }

    func getThumbnail(_ photoId: String, thumbnailWidth: Int, thumbnailHeight: Int, quality: Float, cropRect: CGRect, completion: @escaping (_ result: PictureData?) -> Void) {
        let parsedId = parseId(id: photoId)
        if (parsedId[0].count > 10) { // id is prior
            let fetchResult = PHAsset.fetchAssets(withLocalIdentifiers: [parsedId[0]], options: self.fetchOptions)
            ...
            fetchResult.enumerateObjects({
                ...
                self.cachingImageManager.fetchImage(for: asset, cropRect: cropRect, targetSize: CGSize(width: width, height: height)) {
                    ...
                    let imageData = PhotoLibraryService.image2PictureData(image.resized(to: CGSize(width: thumbnailWidth, height: thumbnailHeight))!, quality: quality)
            ...
            var image: UIImage?
            if (parsedId[0] == "image" || parsedId[0] == "img") {
                if (FileManager.default.fileExists(atPath: parsedId[1])) {
                    image = UIImage(contentsOfFile: parsedId[1])!
                }
            } else { // video from camera is already cropped
                image = thumbnailFromVideoPath(URL(fileURLWithPath: parsedId[1]))
            }
    ...
    func getPhoto(_ photoId: String, width: Int, height: Int, quality: Float, cropRect: CGRect, completion: @escaping (_ result: PictureData?) -> Void) {
        let parsedId = parseId(id: photoId)
        if (parsedId[0].count > 10) { // id is prior
            let fetchResult = PHAsset.fetchAssets(withLocalIdentifiers: [parsedId[0]], options: self.fetchOptions)
            ...
            fetchResult.enumerateObjects({
                ...
                if (asset.mediaType == PHAssetMediaType.video) {
                    PHImageManager.default().requestAVAsset(forVideo: asset, options: nil, resultHandler: { (avAsset: AVAsset?, audioMix: AVAudioMix?, info: [AnyHashable: Any]?) in

                        let asset = avAsset as! AVURLAsset

                        self._getVideoDataFor(asset, completion: { videoData in
                            completion(PictureData(data: videoData!, mimeType: "video/mp4"))
                        })
                    })
                } else {
                    PHImageManager.default().requestImageData(for: asset, options: self.imageRequestOptions) {
            ...
            if (parsedId[0] == "image" || parsedId[0] == "img") {
                if (FileManager.default.fileExists(atPath: parsedId[1])) {
                    let image = UIImage(contentsOfFile: parsedId[1])!
                    let picData = PhotoLibraryService.image2PictureData(image.resized(to: CGSize(width: width, height: height))!, quality: quality)
                ...
                self._getVideoDataFor(URL(fileURLWithPath: parsedId[1]), completion: { videoData in
                    completion(PictureData(data: videoData!, mimeType: "video/mp4"))
                })
            }
        }
    ...
    private func _getVideoDataFor(_ item: AVURLAsset, completion: @escaping (Data?) -> ()) {
        guard item.isExportable else {
            completion(nil)
            return
        }

        let compatiblePresets = AVAssetExportSession.exportPresets(compatibleWith: item)
        var preset: String = AVAssetExportPresetPassthrough
        if compatiblePresets.contains(AVAssetExportPreset1920x1080) { preset = AVAssetExportPreset1920x1080 }

        guard
            let exportSession = AVAssetExportSession(asset: item, presetName: preset),
        ...
        exportSession.exportAsynchronously {
            if exportSession.status == .completed {
                ...
                print("PhotoLibraryService -> Export of the video failed. Reason: \(String(describing: exportSession.error))")
            }
            completion(nil)
            return
        }
    ...
    private func _getVideoDataFor(_ url: URL, completion: @escaping (Data?) -> ()) {
        // input file
...
//                        let mime_type = self.mimeTypes[url.pathExtension.lowercased()]
    ...
    static func image2PictureData(_ image: UIImage, quality: Float) -> PictureData? {
