...
public class SnackBarView extends RelativeLayout {
    ...
    private void init() {
        View.inflate(getContext(), FakeR.staticGetId("layout", "ip_snackbar"), this);
        if (isInEditMode()) {
            return;
        }
        int height = getContext().getResources().getDimensionPixelSize(FakeR.staticGetId("dimen", "ip_height_snackbar"));
        ViewCompat.setTranslationY(this, height);
        ViewCompat.setAlpha(this, 0f);

        int padding = getContext().getResources().getDimensionPixelSize(FakeR.staticGetId("dimen", "ip_spacing_double"));
        setPadding(padding, 0, padding, 0);

        txtCaption = (TextView) findViewById(FakeR.staticGetId("id", "ip_snackbar_txt_bottom_caption"));
        btnAction = (Button) findViewById(FakeR.staticGetId("id", "ip_snackbar_btn_action"));
    }
    ...
    public void setOnActionClickListener(@StringRes int textId, final OnClickListener onClickListener) {
        if (textId == 0) {
            textId = FakeR.staticGetId("string", "ip_ok");
        }
